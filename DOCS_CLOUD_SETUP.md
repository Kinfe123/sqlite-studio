...
- `docs.config.ts` enables runtime analytics. If the repo already has `docs.config.tsx`, this PR updates that file instead.
...
When `NEXT_PUBLIC_DOCS_CLOUD_PROJECT_ID` is present, the docs runtime attaches Docs Cloud's event handler automatically. Existing `analytics.onEvent` callbacks keep running.
